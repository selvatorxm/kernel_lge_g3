/*
 * drivers/cpufreq/cpufreq_artx.c (CUSTOM INTERACTIVE BY ARTX)
 *
 ...
 *
 * Author: Jesus Acosta (jesusgabriel.91@gmail.com)
 * Original Author: Mike Chan (mike@android.com)
 *
 */
...
#include <trace/events/cpufreq_artx.h>

static int active_count;

struct cpufreq_artx_cpuinfo {
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ARTX
static
#endif
struct cpufreq_governor cpufreq_gov_artx = {
	.name = "artx",
	.governor = cpufreq_governor_artx,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_artx_timer_resched(
	struct cpufreq_artx_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_artx_timer_start(int cpu)
{
	struct cpufreq_artx_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	unsigned long flags;
...
static unsigned int choose_freq(
	struct cpufreq_artx_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_artx_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
...
/*		if (sync_freq && new_freq < sync_freq) {
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_artx_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_artx_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_artx_timer_resched(pcpu);
...
static void cpufreq_artx_idle_start(void)
{
	struct cpufreq_artx_cpuinfo *pcpu =
...
static void cpufreq_artx_idle_end(void)
{
	struct cpufreq_artx_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_artx_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_artx_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_artx_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_artx_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_artx_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_artx_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_artx_notifier,
};
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_artx_boost("pulse");
	cpufreq_artx_boost();
	return count;
}
...
static struct attribute_group artx_attr_group = {
	.attrs = artx_attributes,
	.name = "artx",
};

static int cpufreq_artx_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_artx_idle_start();
		break;
	case IDLE_END:
		cpufreq_artx_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_artx_idle_nb = {
	.notifier_call = cpufreq_artx_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		idle_notifier_register(&cpufreq_artx_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_artx_idle_nb);
...
static void cpufreq_artx_nop_timer(unsigned long data)
{
}

static int __init cpufreq_artx_init(void)
{
	unsigned int i;
	struct cpufreq_artx_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_artx_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_artx_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_artx_speedchange_task, NULL,
			       "cfartx");
	...
	return cpufreq_register_governor(&cpufreq_gov_artx);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ARTX
fs_initcall(cpufreq_artx_init);
#else
module_init(cpufreq_artx_init);
#endif

static void __exit cpufreq_artx_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_artx);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_artx_exit);

MODULE_AUTHOR("Jesus Acosta <jesusgabriel.91@gmail.com>");
MODULE_DESCRIPTION("'cpufreq_artx' - A Custom Interactive governor for "
	"Perfomance and battery balance");
MODULE_LICENSE("GPL");
